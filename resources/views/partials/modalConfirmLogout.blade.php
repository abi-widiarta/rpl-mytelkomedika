<div x-data="{ isOpen: false }" class="relative flex">
  ...
  >
      <div id="modal-logout-container" class="items-end justify-center hidden min-h-screen px-4 pt-4 pb-20 text-center transition-all duration-500 sm:block sm:p-0">
          <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>

          <div id="modal-logout" class="relative hidden inline-block px-10 pt-5 pb-4 overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl modal-reservasi sm:my-8 sm:align-middle">
              <div class="flex flex-col items-center justify-center w-full h-full ">
                  <div class="mt-2 mb-4 text-center">
                      ...
                      <form class="flex justify-center gap-6" action="/logout" method="POST">
                        @csrf
                              <a
                                  @click="isOpen = false"
...
<script defer>
  ...
  let modalContainerLogout = document.querySelector("#modal-logout-container")
  setTimeout(() => {
    modalLogout.classList.remove("hidden")
  }, 250);

  setTimeout(() => {
    modalContainerLogout.style.backgroundColor = "rgba(0,0,0,0.2)"
  }, 250);
